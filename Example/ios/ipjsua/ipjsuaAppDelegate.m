...
//    [self.window makeKeyAndVisible];
    ...
    UIStoryboard *mainStoryboard = [UIStoryboard storyboardWithName:@"Main"
                                                             bundle: nil];
    self.controller = (UINavigationController*)[mainStoryboard instantiateViewControllerWithIdentifier: @"viewController"];
...
//    [NSThread detachNewThreadSelector:@selector(pjsuaStart) toTarget:self withObject:nil];
    ...
    _cppwrapper = [[CPPWrapper alloc] init];
    [_cppwrapper createLibWrapper];
    [_cppwrapper incoming_call_wrapper:(void (*))^(int x){   NSLog(@"hello"); printf("hello2");    }];
    
    
    //[_cppwrapper EventCallbackFunction_wrapper:_method1];

    [self performSelector:@selector(reinviteCall) withObject:nil afterDelay:2];
    
   // [self incomingCallView];
    //[self outgoingCallView];
...
-(void)registerStatus
{
    if ([_cppwrapper registerStateInfoWrapper]){
//        _statusLabel.text = @"Sip Status: REGISTERED";
        NSLog(@"Sip Status: REGISTERED");
        
        //_method1 = [self outgoingCallView:(int)];
//        void (^printXAndY)(int) = ^(int y) {
//
//        };
//        CPPWrapper *cppwrapper = [[CPPWrapper alloc]init];
//        [cppwrapper outgoingCallWrapper:@"sip:918313500075@54.211.123.24:20561;transport=tls"];
//        [cppwrapper call_listener_wrapper:printXAndY];

      //  [[NSNotificationCenter defaultCenter] postNotificationName:@"outboundCalling" object:nil];
       // [[NSNotificationCenter defaultCenter] postNotificationName:@"inboundCalling" object:nil];

        
    } else {
//       _statusLabel.text = @"Sip Status: NOT REGISTERED";
        NSLog(@"Sip Status: NOT REGISTERED");
    }
}

-(void)reinviteCall
{
    [_cppwrapper createAccountWrapper:@"19001231232" :@"75214" :@"3.212.140.250" :@"20561"];
    [self performSelector:@selector(registerStatus) withObject:nil afterDelay:2];
    //[sipCall checkIfAudioPresent];
//    [[[VialerSIPLib sharedInstance] callManager] reinviteActiveCallsForAccount:[[CoreDataManager sharedManager] account]];
}
...
-(void)incomingCallView{
    ...
    UIStoryboard *mainStoryboard = [UIStoryboard storyboardWithName:@"Main"
                                                                     bundle: nil];
    self.controller = (UINavigationController*)[mainStoryboard instantiateViewControllerWithIdentifier:@"incomingCallVC"];
    self.window.rootViewController = self.controller;
    [self.window makeKeyAndVisible];
}

-(void)outgoingCallView:(int)callCode{
    self.window = [[UIWindow alloc] initWithFrame:[[UIScreen mainScreen] bounds]];
    UIStoryboard *mainStoryboard = [UIStoryboard storyboardWithName:@"Main"
                                                                     bundle: nil];
    self.controller = (UINavigationController*)[mainStoryboard instantiateViewControllerWithIdentifier:@"outgoingCallVC"];
    self.window.rootViewController = self.controller;
    [self.window makeKeyAndVisible];
}
