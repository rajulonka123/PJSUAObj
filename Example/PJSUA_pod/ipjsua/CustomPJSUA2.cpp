...
void PJSua2::createLib() {
    ...
    TransportId tid = ep->transportCreate(PJSIP_TRANSPORT_TLS, tcfg);
        ep->codecSetPriority("GSM/8000", 210);
...
void PJSua2::createAccount(std::string username, std::string password, std::string ip, std::string port) {
    ...
    AccountConfig acfg;
    acfg.idUri = "sip:" + username + "@" + ip + ":" + port+";transport=tls";
    acfg.regConfig.registrarUri = "sip:" + ip + ":" + port+";transport=tls";
    //acfg.sipConfig.proxies = { "sip:54.211.123.24" };
    ...
    acfg.natConfig.turnUserName = "LinkedPhone7867gyu";
    acfg.natConfig.turnPassword = "hjfytrdrytfjguyhjgytur76";
    acfg.natConfig.turnServer = "turn:54.159.41.210:5349";
    acfg.natConfig.contactRewriteMethod = PJSUA_CONTACT_REWRITE_ALWAYS_UPDATE;
...
void PJSua2::EventCallbackFunction(void (* funcpntr)()){
    printf("event call function");
}
